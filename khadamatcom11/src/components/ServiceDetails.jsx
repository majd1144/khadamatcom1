...
export default function ServiceDetails() {
  ...
  useEffect(() => {
    axios.get(`http://localhost:4000/workers?servicecategory=${servicecategory}`)
      .then((response) => {
        setWorkers(response.data); // نخزن كل العمّال
        setLoading(false);
      })
      .catch((error) => {
      ...
      <div className="services-list">
        {workers.map((worker) => (
          <div className="card" key={worker.id} style={{ width: '400px' }}>
            ...
            <br />
            <div className="card-body">
              <h3 className="card-title" style={{ fontFamily: 'roman' }}>{worker.name}</h3>
              <br />
              <h4 className="card-subtitle">The job: {worker.servicecategory}</h4>
              <br />
              <h4 className="card-subtitle">Location: {worker.location}</h4>
